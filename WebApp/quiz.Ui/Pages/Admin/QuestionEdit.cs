...
/// <summary>
/// Редактор данных Вопроса.
/// </summary>
public partial class QuestionEdit : IDisposable
{
...
#pragma warning disable CS8618
    ...
    /// </summary>
    [Inject]
    private IObjectDataService<QuestionModel> QuestionDataService { get; set; }
...
#pragma warning restore CS8618
    ...
    /// Перечень всех Тем.
    /// </summary>
    private IEnumerable<TopicModel> Topics { get; set; } = null!;

    /// <summary>
    /// Переменная содержит данные редактируемого объекта.
    /// </summary>
    private QuestionModel? EditingObject { get; set; }
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        // Получение всех Тем через WebAPI. 
        WebApiCallResult<IEnumerable<TopicModel>> webApiCallResultTopics = await TopicDataService.GetAllObjectsAsync(currentUserName);

        // Если в процессе получения данных от WebAPI были зарегистрированы ошибки, то
        if (webApiCallResultTopics.HasErrors)
        {
            // Вывод пользователю информации об этом.
            HasErrors = webApiCallResultTopics.HasErrors;
            ResultDescription = webApiCallResultTopics.ResultDescription;
            // Дальнейшее выполнение функции прекращается.
            return;
        }
        
        // Получение данных редактируемого объекта из результата обращения к WebAPI.
        Topics = webApiCallResultTopics.Data.OrderBy(i => i.OrderNumber);
    }
    ...
    protected override async Task OnParametersSetAsync()
    {
        ...
        if (Id != null)
        {
            ...
            WebApiCallResult<QuestionModel> webApiCallResult = await QuestionDataService.GetObjectByIdAsync(Id.Value, currentUserName);
            ...
            HeaderText = $"Изменение данных Вопроса \"{EditingObject?.QuestionText}\"";
        }
        else
        {
            ...
            // Установка текста заголовка страницы редактора.
            HeaderText = "Добавление нового Вопроса";
            // Id первой темы из всех.
            Guid firstTopicId = Guid.Empty;
            // Получение из списка тем первой.
            TopicModel? firstTopic = Topics.MinBy(i => i.OrderNumber);

            // Если удалось получить первую тему из всех, то
            if (firstTopic != null)
            {
                // Получение Id первой из всех тем.
                firstTopicId = firstTopic.Id;
            }

            // Создание нового пустого объекта.
            EditingObject = new QuestionModel(
                Guid.NewGuid(),
                firstTopicId,
                string.Empty,
                string.Empty,
    ...
    private async Task HandleValidSubmitAsync()
    {
        ...
        if (EditingObject != null)
        {
            ...
            if (_newObjectCreating)
            {
                ...
                WebApiCallResult<QuestionModel> webApiCallResult = await QuestionDataService.AddObjectAsync(EditingObject, currentUserName);
            ...
            else
            {
                ...
                WebApiCallResult<QuestionModel> webApiCallResult = await QuestionDataService.UpdateObjectAsync(EditingObject, currentUserName);
